...
public class WebRequestUtil {
        ...
        if (hc == null) {
            synchronized (WebRequestUtil.class) {
                if (hc == null) {
                    ...
                    rc = RequestConfig.custom().setCookieSpec(CookieSpecs.IGNORE_COOKIES).setSocketTimeout(SOCKET_TIMEOUT)
                            .setConnectTimeout(CONNECTION_TIMEOUT).setConnectionRequestTimeout(CONNECTION_REQUEST_TIMEOUT)
                            .setExpectContinueEnabled(false).setRedirectsEnabled(false).build();
                }
            }
        }
    ...
    private static CloseableHttpResponse getHttpResponse(String baseUrl, String params, String cid, boolean useGzip) throws ClientProtocolException,
            IOException {
