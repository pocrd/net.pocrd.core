...
public class DemoDao<T> {
	...
	public DemoEntity getDemoEntity(
			...
			@CacheParameter(type = CacheKeyType.Normal)String condition2,
			@CacheParameter(type = CacheKeyType.Normal)boolean conditionBool,
			@CacheParameter(type = CacheKeyType.Normal)byte condtionByte,
			@CacheParameter(type = CacheKeyType.Normal)short condtionShort,
			@CacheParameter(type = CacheKeyType.Normal)char condtionChar,
			@CacheParameter(type = CacheKeyType.Normal)long condtionLong,
			@CacheParameter(type = CacheKeyType.Normal)float condtionFloat,
			@CacheParameter(type = CacheKeyType.Normal)double condtionDouble
			) {
		if (condition == 0) {
...
//			System.out.println("Success 1");
	...
	@CacheMethod(enable = true, expire = 100, key = "getDemoEntity")
	public int[] getDemoEntity(
			@CacheParameter(type = CacheKeyType.Normal)double conditionDouble,
	 ...
	 */
	@CacheMethod(enable = true, expire = 100, key = "getDemoEntity")
	public List<String> getDemoEntity(
			@CacheParameter(type = CacheKeyType.Normal) List<String> condition) {
		//System.out.println("Success 5");
		return null;
	}
