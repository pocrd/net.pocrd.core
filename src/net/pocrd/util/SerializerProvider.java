...
public class SerializerProvider implements Opcodes {
    ...
    private static <T> Serializer<T> build(Class<T> clazz) {
        ...
        try {
            ...
            {
                mv = cw.visitMethod(ACC_PUBLIC, "toJson", "(" + t_classDesc + "Ljava/io/OutputStream;)V", null, null);
            ...
            {
                mv = cw.visitMethod(ACC_PUBLIC + ACC_BRIDGE + ACC_SYNTHETIC, "toJson", "(Ljava/lang/Object;Ljava/io/OutputStream;)V", null, null);
                mv.visitCode();
                mv.visitVarInsn(ALOAD, 0);
                mv.visitVarInsn(ALOAD, 1);
                mv.visitTypeInsn(CHECKCAST, t_className);
                mv.visitVarInsn(ALOAD, 2);
                mv.visitMethodInsn(INVOKEVIRTUAL, c_name, "toJson", "(" + t_classDesc + "Ljava/io/OutputStream;)V");
                mv.visitInsn(RETURN);
                mv.visitMaxs(3, 3);
                mv.visitEnd();
            }
            ...
            if (CommonConfig.isDebug) {
                ...
                try {
                    File folder = new File(CommonConfig.Instance.autogenPath + "\\Serializer\\");
                    if (!folder.exists()) {
                        folder.mkdirs();
                    }
                    fos = new FileOutputStream(CommonConfig.Instance.autogenPath + "\\Serializer\\" + className + ".class");
    ...
    private static void buildToXml(MethodVisitor mv, Class<?> clazz, List<FieldDescriptor> fds, String classDesc, String t_className,
            String t_classDesc) throws SecurityException, NoSuchMethodException, ClassNotFoundException {
        int newLocalIndex = 0;
            ...
            switch (t) {
                ...
                case DOUBLE:
                    ...
                    if (fd.isRepeated()) {
                        ...
                        Label lable_toLoop = new Label();
                        mv.visitJumpInsn(GOTO, lable_toLoop);
                        Label lable_loop = new Label();
                        mv.visitLabel(lable_loop);
                        ...
                        mv.visitLabel(lable_toLoop);
                        mv.visitVarInsn(ILOAD, local_i);
                        mv.visitVarInsn(ILOAD, local_size);
                        mv.visitJumpInsn(IF_ICMPLT, lable_loop);
                ...
                case STRING:
                    if (fd.isRepeated()) {
                        ...
                        // mv.visitMethodInsn(INVOKEVIRTUAL, "java/lang/String", "getBytes", "(Ljava/nio/charset/Charset;)[B");
                        // mv.visitMethodInsn(INVOKEVIRTUAL, "java/io/OutputStream", "write", "([B)V");
                        mv.visitMethodInsn(INVOKESTATIC, "net/pocrd/util/SerializerProvider", "writeString",
    ...
    private static void buildToJson(MethodVisitor mv, Class<?> clazz, List<FieldDescriptor> fds, String classDesc, String t_className,
        ...
        mv.visitLocalVariable("instance", t_classDesc, null, l0, l1, 1);
        mv.visitLocalVariable("out", "Ljava/io/OutputStream;", null, l0, l1, 2);
        mv.visitMaxs(0, 3);
    }
